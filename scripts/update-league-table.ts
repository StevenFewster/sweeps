...
async function fetchBBCTable(): Promise<{ teams: Team[], gamesPlayed: number, totalGoals: number }> {
  ...
  try {
    ...
    rows.forEach((row, index) => {
      ...
      const played = parseInt(Array.from(cells).find((cell, idx) => idx > 0 && idx < 5)?.textContent?.trim() || '0');
      const goalsFor = parseInt(Array.from(cells).filter(cell => {
        const text = cell.textContent?.trim() || '';
        return !isNaN(Number(text)) && Number(text) >= 0;
      })[4]?.textContent?.trim() || '0');
      const goalsAgainst = parseInt(Array.from(cells).filter(cell => {
        const text = cell.textContent?.trim() || '';
        return !isNaN(Number(text)) && Number(text) >= 0;
      })[5]?.textContent?.trim() || '0');
...
async function updateLeagueTable() {
  try {
    ...
    const estimatedTotalGoals = gamesPlayedSoFar > 0 
